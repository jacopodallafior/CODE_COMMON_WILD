...
class ModeSwitchNode(Node):
    def __init__(self):
        ...
        self.rc_debug_publisher = self.create_publisher(Float32MultiArray, 'rc_steer_debug', 10)
        ...
        if self.mode == 'MANUAL' and self.armed:
            ...
            if (now - self._last_debug_print_time) >= 0.10:
                ...
                self.get_logger().info(
                    f'RC steer_raw={rc_msg.channels[3]:.0f}, '
                    f'target_rate={target_rate:.2f} deg/s, '
                    f'manual_target={self._manual_target_angle:.2f} deg, '
                    f'vel={vel:.2f} m/s'
                )

            dbg = Float32MultiArray()
            dbg.data = [
                float(rc_msg.channels[3]),          # raw steering stick
                float(rc_msg.channels[1]),          # raw speed stick
                float(target_rate),                 # commanded target rate
                float(self._manual_target_angle)    # commanded target angle
            ]
            self.rc_debug_publisher.publish(dbg)
