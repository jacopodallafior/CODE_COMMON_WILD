...
bool   newGPSSample    = false; // set true on every CAN_GPS_VELOCITY, cleared after compute()

// Arm warmup: ignore first N GPS samples after arm to avoid throttle spike
const int armWarmupSamples = 5;  // ~500ms at 10 Hz
int warmupCounter = 0;
...
const float parkingBrakePct     = 60.0;   // % — holds vehicle at standstill
...
void setup() {
  ...
  pidController.setWindUpLimits(-5, 5);
  pidController.start();
...
void loop() {
  ...
  while (mcp2515.readMessage(&canMsg) == MCP2515::ERROR_OK) {
    if (canMsg.can_id == CAN_HB_STERFBOARD) {
      ...
      memcpy(&rv, canMsg.data, 4);
      rawGPSVelocity = rv;
      // First sample initialises filter directly to avoid ramp from 0
  ...
  receivedHeartbeat = (lastHbReceived != 0) &&
                      ((nowMS - lastHbReceived) < (heartbeatInterval * 4UL)); // was 2
  ...
  if (!receivedHeartbeat || mode == 2) {
    ...
    sendBrakePct(100.0);
    controlOutput = 0;
    pidController.stop();   // clears integrator
    pidController.start();
    newGPSSample  = false;
    warmupCounter = armWarmupSamples;  // reset warmup for next arm
    ...
    if (newGPSSample) {
      if (warmupCounter > 0) {
        // First N samples after arm: hold output at zero, let the vehicle
        // settle before the PID starts driving throttle/brake.
        warmupCounter--;
        controlOutput = 0;
        Debug.print("Warmup remaining: ");
        Debug.println(warmupCounter);
      } else {
    ...
    if (controlOutput >= 0) {
      ...
      if (targetVelocity <= 0) {
        // Zero velocity command: no throttle, apply parking brake
        pwmValue = minPWM;
        controlOutput = 0;
        analogWrite(accelPin, pwmValue);

        if (fabs(actualGPSVelocity) <= nearStandstillSpeed) {
          sendBrakePct(parkingBrakePct);  // 60%
