...
void recalcMaxSteps() { maxSteps = (long)(MICROSTEPS * cfgMaxTurns); }
...
void loop() {
  ...
  if (mcp2515.readMessage(&canMsg) == MCP2515::ERROR_OK) {
    ...
    if (canMsg.can_id == CAN_HB_STERFBOARD) {       // 0x01
      ...
      if (mode == 1)
        ...
        Debug.println("Ignored — not AUTO");
    }
  }
  ...
  if (now - lastHbReceived > heartbeatInterval * 2) {
    if (motorEnabled) {
      Serial.println("WATCHDOG: releasing brake");
      applyBrakePct(0.0);
      disableMotor();
    }
  }
...
void handleSerialCommand(String raw) {
  String cmd = raw;
  cmd.toUpperCase();
  if (cmd.startsWith("B:"))     { applyBrakePct(constrain(cmd.substring(2).toFloat(),0,100)); return; }
  if (cmd.startsWith("JOG:"))   {
    int j = cmd.substring(4).toInt();
    enableMotor();
    long t = stepper.currentPosition() + j;
    stepper.moveTo(t); targetSteps = t;
    return;
  }
  if (cmd.startsWith("SPEED:")) { float v=cmd.substring(6).toFloat(); if(v>0&&v<=3000){cfgMaxSpeed=v;stepper.setMaxSpeed(v);} return; }
  if (cmd.startsWith("ACCEL:")) { float v=cmd.substring(6).toFloat(); if(v>0&&v<=5000){cfgAccel=v;stepper.setAcceleration(v);} return; }
  if (cmd.startsWith("TURNS:")) { float v=cmd.substring(6).toFloat(); if(v>0&&v<=5.0){cfgMaxTurns=v;recalcMaxSteps();} return; }
  if (cmd == "SETHOME")  { stepper.setCurrentPosition(0); targetSteps=0; Serial.println("HOME SET"); return; }
  if (cmd == "H")        { enableMotor(); stepper.moveTo(0); targetSteps=0; return; }
  if (cmd == "OFF")      { disableMotor(); return; }
  if (cmd == "CONFIG")   {
  ...
  if (cmd == "POS") return; // already printing every 500ms
  Serial.print("ERR: "); Serial.println(raw);
}