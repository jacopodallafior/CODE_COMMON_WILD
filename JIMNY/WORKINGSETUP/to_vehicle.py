...
"""
to_vehicle_2usb_minimal.py

Minimal change from the old to_vehicle.py for the two-USB architecture.

Goal:
  - keep the old safety/topic behavior as much as possible
  - keep publishing /feedback_data for the old mode_switch.py
  - keep sending H,<0/1/2> to sterfBoard
  - keep the old 6-field serial packet format
  - force steering fields to 0.0 so sterfBoard does not command steering

Use:
  python3 to_vehicle_2usb_minimal.py /dev/ttyUSB0
"""
...
class AckermannToVehicleNode(Node):
    def __init__(self, send_to_serial=True, is_esp32=False, serial_port='/dev/ttyUSB0', baud_rate=115200):
        ...
        if send_to_serial:
            try:
                ...
                if is_esp32:
                    ...
                    self.get_logger().info(f'Connected to sterfBoard/Arduino on {serial_port}')
        ...
        # Keep old mode_switch compatibility.
        # Old mode_switch subscribes to /feedback_data and expects 4 fields:
        # [velocity_ctrl, velocity_fb, steering_rate_ctrl, steering_angle_fb]
    ...
    def ackermann_callback(self, msg):
        # Minimal split change:
        # - keep old immediate-send behavior
        # - ignore steering fields for sterfBoard
        # - still use velocity/acceleration/jerk/GPS exactly as before
        steering_angle = 0.0
        steering_angle_velocity = 0.0
        velocity = float(msg.speed)
        acceleration = float(msg.acceleration)
        jerk = float(msg.jerk)
        ...
        if (now - self._last_cmd_print_time) >= 0.10:
            ...
            self.get_logger().info(
                f'CMD_TO_STERF steer=0.00 deg, steer_rate=0.00 deg/s, '
                f'vel={velocity:.2f} m/s, acc={acceleration:.2f}, '
                f'gps={gps_speed:.2f}, safety={self.ack_signal_flag}'
            )
    ...
    def signal_flag_callback(self, msg):
        if msg.key not in ("0", "1", "2"):
            self.ack_signal_flag = 2
            return
...
def main(args=None):
    try:
        ...
        send_to_serial = True
        serial_port = '/dev/ttyUSB0'   # sterfBoard port
        baud_rate = 115200
        ...
        if 'node' in locals() and node.send_to_serial:
