 ...
 * * ADDED: Slew Rate Limiter for smooth RC joystick control
 */
...
// --- ADDED: SLEW RATE LIMITER GLOBALS ---
float targetSlewed = 0.0;
const float MAX_TARGET_RATE = 30.0; // deg/s max ramp speed. Tune this!
// ----------------------------------------
...
void resetPID() {
  ...
  // --- ADDED: Reset slew to current physical angle ---
...
void loop() {
  ...
  if (mcp2515.readMessage(&canMsg) == MCP2515::ERROR_OK) {
    if (canMsg.can_id == CAN_HB_STERFBOARD) {
      ...
      // --- MODIFIED: Enable PID for both Mode 1 (Auto) and Mode 2 (Manual) ---
      ...
      // --- MODIFIED: Accept CAN targets in both modes ---
      if (mode == 1 || mode == 2) {
  ...
  if (now - lastHbReceived > heartbeatInterval * 4) {
    if (pidEnabled) {
      pidEnabled = false;
      stopOutput();
...
void runControl() {
  ...
  // --- ADDED: Slew rate limit the target ---
  ...
  // --- MODIFIED: Calculate error based on SLEWED target, not raw target ---
  float error = targetSlewed - measuredAngleDeg;
