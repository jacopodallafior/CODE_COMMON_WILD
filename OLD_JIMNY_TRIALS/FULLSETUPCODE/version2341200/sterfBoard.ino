...
const uint32_t commandInterval   = 100;   // 10 Hz
...
void loop() {
  ...
  if (now - lastSerialHeartbeatTime >= 2 * heartbeatInterval) // era 2
...
int computeCheckedMode(unsigned long now) {
  ...
  if (ENABLE_BOARD_FAULTS) {
    // Pulisce latch se operator disarma OPPURE tutte le board tornano vive
    ...
    // Triggera latch se in AUTO e manca qualcosa
    ...
    // Applica latch solo se ancora in AUTO (non sovrascrive MANUAL/DISARMED)
